...
class ComponentOnPageHasValueRepository extends EntityRepository
{
    ...
    private function getActiveComponentBySlugAndLocaleQb($slug, $locale)
    {
        return $this->createQueryBuilder('a')
            ...
            ->addSelect("IF(chc.columnType=9, (SELECT art.slug FROM MyCMSBundle:Article AS art WHERE art.id=a.content), 0) AS article")
            ->addSelect("IF(chc.columnType=10, (SELECT f.path FROM MyFileBundle:File AS f WHERE f.id=a.content), 0) AS file")
            ->join('a.componentOnPageHasElement', 'cophe')
            ->join('a.componentHasColumn', 'chc')
